...
import CombinedDataTable, { EmployeeTableColumns } from './CombinedDataTable';
import SummaryDataTable, { SummaryTableColumns } from './SummaryDataTable';
import { EmployeeGroupColumns } from './EmployeeSortControls';
import { BadgeFilterState } from './FilterControls';
import { SortDirection } from '../utils/displayUtils';
import { CombinedDataRow, SummaryDataRow, EmployeeGroup } from '../types';
...
const getDateRowClass = (dateString: string, isHoliday: boolean): string => {
...
const renderHoursWithDetails = (row: any) => {
  if (!row) return null;
  ...
  return (
    <div className="hours-container">
      {parseFloat(nightHours) > 0 && (
        <div className="hours-item hours-secondary">
          <span className="hours-label">Night:</span>
          <span className="text-info">{nightHours}</span>
        </div>
      )}
      {parseFloat(sundayHours) > 0 && (
        <div className="hours-item hours-secondary">
          <span className="hours-label">Sunday:</span>
          <span className="text-primary">{sundayHours}</span>
        </div>
      )}
      {parseFloat(holidayHours) > 0 && (
        <div className="hours-item hours-secondary">
          <span className="hours-label">Holiday:</span>
          <span className="text-danger">{holidayHours}</span>
        </div>
      )}
      <div className="hours-item hours-primary">
        <span className="hours-label-primary">Total:</span>
...
export default function DataDisplay(): JSX.Element {
  ...
  const [activeTab, setActiveTab] = useState<'combined' | 'summary'>('combined');
  ...
  // Badge filters state
  const [badgeFilters, setBadgeFilters] = useState<BadgeFilterState>({
    correct: true,
    late: true,
    missing: true,
    dayOff: true,
  ...
  // Sorting state
  const [summarySort, setSummarySort] = useState<{column: SummaryTableColumns, direction: SortDirection}>({
    column: 'DIFFERENCE_HOURS',
    direction: 'desc'
  });
  const [employeeSort, setEmployeeSort] = useState<{column: EmployeeTableColumns, direction: SortDirection}>({
    column: 'Date',
    direction: 'asc'
  });
  const [employeeGroupSort, setEmployeeGroupSort] = useState<{column: EmployeeGroupColumns, direction: SortDirection}>({
    column: 'AFM',
    direction: 'asc'
  });

  // Toggle badge filter
  const toggleBadgeFilter = (filter: keyof BadgeFilterState) => {
  ...
  const toggleAllFilters = (state: boolean) => {
    setBadgeFilters({
      correct: state,
      late: state,
      missing: state,
  ...
  const handleSummarySort = (column: SummaryTableColumns) => {
    setSummarySort(prev => ({
      column,
  ...
  const handleEmployeeSort = (column: EmployeeTableColumns) => {
  ...
  const handleEmployeeGroupSort = (column: EmployeeGroupColumns) => {
  ...
  const downloadCsv = useCallback((data: CombinedDataRow[] | SummaryDataRow[], filename: string) => {
    ...
    return combined
      .filter(row => {
        ...
        if (searchTerm) {
          ...
          return (
            (row.AFM?.toString() || '').toLowerCase().includes(searchLower) ||
  ...
  const sortEmployeeData = useCallback((data: CombinedDataRow[], column: EmployeeTableColumns, direction: SortDirection): CombinedDataRow[] => {
  ...
  const groupedData = useMemo(() => {
    ...
    const groups: Record<string, CombinedDataRow[]> = {};
  ...
  return (
    <div className="data-display">
      ...
      <div className="tab-content">
        {/* Combined Data Tab */}
      <div className={activeTab === 'combined' ? '' : 'd-none'}>
        {combined && (
            <CombinedDataTable 
              ...
              handleEmployeeGroupSort={handleEmployeeGroupSort}
        ...
        {/* Summary Data Tab */}
      <div className={activeTab === 'summary' ? '' : 'd-none'}>
      {summary && (
            <SummaryDataTable
              summaryData={filteredSummary}
              paginatedSummary={paginatedSummary}
              summaryPage={summaryPage}
              setSummaryPage={setSummaryPage}
              summaryPerPage={summaryPerPage}
              setSummaryPerPage={setSummaryPerPage}
              summarySearch={summarySearch}
              setSummarySearch={setSummarySearch}
              summarySort={summarySort}
              handleSummarySort={handleSummarySort}
              downloadCsv={downloadCsv}
            />
          )}
                </div>
